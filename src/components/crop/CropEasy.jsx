...
const CropEasy = ({photoURL, setOpenCrop,  setPhotoURL, setFile}) => {
  ...
  return (
      <>
        <DialogContent dividers sx={{width:"auto", minWidth:{sm:500}, height:400, position:"relative", background:"#333"}}>
            <Cropper image={photoURL} crop={crop} zoom={zoom} rotation={rotation} aspect={1}
            onZoomChange={setZoom} onRotationChange={setRotation} onCropChange={setCrop}
            onCropComplete={cropComplete}
            />
        </DialogContent>
        <DialogActions>
            <Box sx={{flexDirection:"column", mx:3, my:2, width:"95%"}}>
                <Box>
                    <Typography>Zoom: {zoomPercent(zoom)}</Typography>
                    <Slider valueLabelDisplay="auto" valueLabelFormat={zoomPercent} min={1} max={3} step={0.1} 
                    value={zoom} onChange={(e,zoom) => setZoom(zoom)} />
                </Box>
                <Box>
                    <Typography>Rotation: {rotation}</Typography>
                    <Slider valueLabelDisplay="auto" valueLabelFormat={rotation} min={0} max={360}  
                    value={rotation} onChange={(e,rotation) => setRotation(rotation)} />
                </Box>
